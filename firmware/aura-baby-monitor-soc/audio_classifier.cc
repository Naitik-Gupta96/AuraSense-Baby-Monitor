...
static int majority_decide(void)
{
  ...
  if (n_cry < MAJORITY_MIN_CRY_COUNT) return (int)NO_CLASS_ID;  // background
  // Ambiguous pattern (mixed sad/laugh votes in same short window) is
  // usually background/noise for this model revision. Suppress to BG.
  if (n_sad >= 2 && n_laugh >= 2) return (int)NO_CLASS_ID;
  // Also require at least a small dominance margin.
...
{
  if (!device_enabled || !monitoring_enabled) {
...
void audio_classifier_task(void *arg)
{
  ...
  while (1) {
    if (deep_sleep_enabled || !device_enabled) {
